...
- It would be nice to not have such specific settings for the accelerometer and joystick. By this we mean that it would be nice to have the bird fly regardless of which direction you tilt the accelerometer or push the joystick. Currently it only works if you push the joystick up or tilt the accelerometer to the right. For simplicity sake, we kept this interaction as is. But maybe for the next iteration, we could incorporate some leeway such that the user doesn't have to think too much about the setup.
